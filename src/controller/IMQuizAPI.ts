...
export default {
    ...
    getQuestion(topicID: any = null, type: any = AppEnum.GENERAL_CATEGORY_TYPE.Short) {
        return new Promise(async (resolve: ({ data: [], }) => void, reject) => {
            const authToken = await appFunctions.authToken();
            const param = type === AppEnum.GENERAL_CATEGORY_TYPE.Short ?
                `${ApiClasses.allShort}?topic_id=${topicID}&authToken=${authToken}`
                :
                `${ApiClasses.allLong}?topic_id=${topicID}&authToken=${authToken}`;
